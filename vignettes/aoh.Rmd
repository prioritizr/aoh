...
* **Where can I find species range data for generating Area of Habitat data?**
    ...
    Species range data can be obtained from the [IUCN Red List](https://www.iucnredlist.org/) (see [Spatial Data Download resources](https://www.iucnredlist.org/resources/spatial-data-download)). They can also be produced using observation records from data repositories, such as the [Global Biodiversity Information Facility](https://www.gbif.org/) and the [Atlas of Living Australia](https://www.ala.org.au/). Please note that species range data must follow the same data formats as the IUCN Red List. This means that the species range data must contain the following columns: `id_no`, `presence`, `origin`, `seasonal`, `terrestrial` (or `terrestial`), `freshwater`, and `marine`. For further details, please see the **Species range data format** section in the documentation for `create_spp_aoh_data()`.

* **Can I use species range data from other data sources (instead of the IUCN Red List)?**

    Yes, species range data do not necessarily need to be obtained from the IUCN Red List. For example, species range data can be produced using observation records from data repositories [e.g. following @r8], such as the [Global Biodiversity Information Facility](https://www.gbif.org/) and the [Atlas of Living Australia](https://www.ala.org.au/). To generate Area of Habitat data from manually generated species range data, please ensure that the species range data follow the same data formats as the IUCN Red List. This means that the species range data must contain the following columns: `id_no`, `presence`, `origin`, `seasonal`, `terrestrial` (or `terrestial`), `freshwater`, and `marine`. For further details, please see the **Species range data format** section in the documentation for `create_spp_aoh_data()`.
