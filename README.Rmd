...
Finally, let's create some maps to compare the geographic range data with the Area of habitat data. Although we could create these maps manually (e.g. using the [_ggplot2_](https://CRAN.R-project.org/package=rappdirs) _R_ package), we will use a plotting function distributed with the _aoh R_ package for convenience.
