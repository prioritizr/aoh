...
* **Where can I find species range data for generating Area of Habitat data?**
    ...
    Species range data can be obtained from the [IUCN Red List](https://www.iucnredlist.org/) (see [Spatial Data Download resources](https://www.iucnredlist.org/resources/spatial-data-download)). They can also be produced using observation records from data repositories, such as the [Global Biodiversity Information Facility](https://www.gbif.org/) and the [Atlas of Living Australia](https://www.ala.org.au/). Please note that species range data must follow the same data formats as the IUCN Red List.
    This means that the species range data must contain the following columns:
    `id_no`, `presence`, `origin`, `seasonal`, `terrestrial` (or `terrestial`), `freshwater`, and `marine`. For further details, please see the **Species range data format** section in the documentation for `create_spp_aoh_data()`.

* **I want to use a specific habitat classification dataset for generating Area of Habitat data (e.g. [CORRINE land cover dataset](https://land.copernicus.eu/pan-european/corine-land-cover)), how can I do this?**

    The `create_spp_aoh_data()` function uses a multi-layer raster (`terra::rast()`) object to describe the coverage of different habitat classes (per the `habitat_data` parameter). Specifically, each layer in the object should correspond to a different habitat class defined by the [IUCN Red List Habitat Classification Scheme](https://www.iucnredlist.org/resources/habitat-classification-scheme). To identify which layers correspond to which habitat classes, each layer should have a name indicating the relevant IUCN Red List habitat classification code (see `terra::names()` to set names). For example, a layer corresponding to the "Forest - Subarctic" habitat class should have the name `"1.1"`. It is important to ensure that the names of the raster object correspond to the IUCN Red List habitat classification code. To help verify the layer names, the `create_spp_aoh_data()` function will display a warning message if any IUCN habitat codes specified for particular species codes are missing from the argument to `habitat_data` (see tutorial above for an example of this).

    The cell values within each layer should correspond to the percent coverage of each habitat class, rescaled between 0 and 1,000. For example, a value of 1,000 means that a raster cell has 100% coverage by a given habitat class, a value of 500 means that a raster cell has 50% coverage by a given habitat class, and a value of 0 means that a raster cell has 0% coverage by a given habitat class. The reason why these values should range between 0 and 1,000 is because this enables the data to be stored in a certain format that drastically reduces run time and computational requirements (i.e. `INT2U` values, see documentation for the `terra::writeRaster()` function for details). Although the input habitat data should have values ranging between 0 and 1,000; the output Area of Habitat data contain the percent coverage of suitable habitat within each raster cell, scaled between 0 and 1.
